// Driver JSON (mặc định): lưu tất cả vào 1 file <DATA_DIR>/studio.json.
...
// Chuẩn hoá settings + mặc định (dùng chung với driver khác qua export).
...
export function makeJson() {
  ...
  getSettings() { return normalizeSettings(read().settings || {}); },
  setSettings(patch) {
...
// Đọc thô studio.json (dùng cho migrate sang driver DB). Rỗng nếu chưa có file.
export function readRawJson() {
  return existsSync(FILE) ? read() : null;
}
